...
def catalog():
    ...
    dishes = Dish.query.filter(Dish.category_id == 0).order_by(Dish.number.asc()).all()
    return render_template('admin/catalog.html', title='Каталог', area='catalog', categories=categories, dishes=dishes)
...
def create_dish():
    ...
    form.category.choices.insert(0, (0, 'Нет'))
    if form.validate_on_submit():
        ...
        print(new_dish)
        flash('Блюдо {} успешно добавлено'.format(
            name
        ), category='success')
        return redirect(url_for('admin.catalog'))
