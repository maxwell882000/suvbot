...
def back_to_the_catalog(chat_id, language, message_text=None, parent_category=None):
    ...
    #bot.register_next_step_handler_by_chat_id(chat_id, catalog_processor)
    dishes = Dish.query.filter(Dish.category_id == 0, Dish.is_hidden == False).order_by(Dish.number.asc()).all()
    ...
    bot.send_message(chat_id=chat_id, text=dish_message, reply_markup=dishes_keyboard)
